...
def create_chat(user):
    ...
    if old_history is not None:
        for i in range(0, len(old_history) - 1):
            chat_history.append(
                {"role": userRole, "parts": [old_history[i]["message"]]}
            )
            chat_history.append(
                {"role": modelRole, "parts": [old_history[i]["response"]]}
            )
    else:
        chat_history.append(
            {"role": userRole, "parts": ["Hi, what do you want to know about plants?"]}
        )
        chat_history.append(
            {
                "role": modelRole,
...
def chatBot_controller(email):
    ...
    try:
        ...
        if not message:
            return jsonify(
                {"status": False, "message": "Message field is required", "data": None}
            ), 400
        ...
        return jsonify(
            {"status": False, "message": "Model failed to answer.", "data": e}
        ), 404
