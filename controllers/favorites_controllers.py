...
def add_favorites(email):
    try:
        ...
        if not product_id:
            return jsonify(
                {"status": False, "message": "Product id is required", "data": None}
            ), 400
        ...
        return jsonify(
            {"status": False, "message": "Product not added to favorites", "data": e}
        ), 500
...
def get_user_fav_products(email):
    try:
        ...
        if not favorites:
            return jsonify(
                {"status": False, "message": "No favorites found", "data": None}
            ), 404
        return jsonify(
            {
                "status": True,
        ...
        return jsonify(
            {"status": False, "message": "No favorites found", "data": e}
        ), 404
