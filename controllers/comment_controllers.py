...
def create_comment(email):
    ...
    try:
        ...
        add_comment(comment, commenter, product, prediction)
        ...
        return jsonify(
            {"status": False, "message": "comment not added", "data": e}
        ), 404
...
def delete_comment(email):
    ...
    try:
        ...
        return jsonify(
            {"status": True, "message": "comment deleted successfully", "data": None}
        ), 200
    except Exception as e:
        return jsonify(
            {"status": False, "message": "comment not found", "data": e}
        ), 500
...
def get_comments(product):
    ...
    try:
        ...
        if len(comments) == 0:
            return jsonify(
                {"status": False, "message": "No comments found", "data": []}
            ), 200
        return jsonify({"status": True, "message": "success", "data": comments}), 200
    except Exception as e:
        return jsonify(
            {"status": False, "message": "No comments found", "data": e}
        ), 404
