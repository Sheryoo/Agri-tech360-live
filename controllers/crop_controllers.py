...
def recommend_crop():
    ...
    try:
        ...
        confidence, final_class = classify_soil(file)
        season = get_season()

        soil_type = final_class.split(" ")[0].lower()
        previous_crop = request.form.get("previous_crop").strip().lower()

        recommended_crop = recommend_next_crop(soil_type, previous_crop, season)
        ...
        return jsonify(
            {"status": False, "message": "Something went wrong", "data": e}
        ), 500
