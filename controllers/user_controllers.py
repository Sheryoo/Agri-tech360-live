...
def register(app):
    ...
    try:
        ...
        if get_user_by_email(email):
            return jsonify(
                {"status": False, "message": "User already exists", "data": None}
            ), 400

        hashed_password = hash_password(password)
        register_user(email, hashed_password, fullName, city, phoneNumber, profilePic)
        ...
        return jsonify({"status": False, "message": "User not created", "data": e})
...
def login(app):
    ...
    try:
        ...
        if not verify_password(user[3], password):
            return jsonify(
                {"status": False, "message": "Invalid password", "data": None}
            ), 401
        ...
        return jsonify({"status": False, "message": "User not found", "data": e}), 404
...
def update_user(inputData, email):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "UPDATE users SET fullName = ?, city = ?, phoneNumber = ? WHERE email = ?",
...
def update_password(email):
    ...
    try:
        ...
        if not verify_password(user[3], old_password):
            return jsonify(
                {"status": False, "message": "Wrong old password", "data": None}
            ), 401
