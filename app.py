...
from controllers.favorites_controllers import (
    add_favorites,
    delete_favorites,
    get_user_fav_products,
)
...
swagger_ui_blueprint = get_swaggerui_blueprint(
    SWAGGER_URL, API_URL, config={"app_name": "Agri-tech360 API"}
)
...
@auth_token
def get_user_data_api(email):
    return get_user_data(email)


@app.route("/update-user-data", methods=["PUT"])
@auth_token
def update_user_data_api(email):
    return update_user_data(email)


@app.route("/update-password", methods=["PUT"])
@auth_token
def update_password_api(email):
    return update_password(email)


@app.route("/get-weather", methods=["GET"])
@auth_token
def get_weather_api(email):
    return get_weather(email)


@app.route("/plants", methods=["GET"])
@auth_token
def get_plants(email):
    return get_all_plants()


@app.route("/plants/<plant_name>", methods=["GET"])
@auth_token
def get_plant(email, plant_name):
    return get_plant_by_name(plant_name)


@app.route("/classify", methods=["POST"])
@auth_token
def classify_api(email):
    return classify_image()


@app.route("/add-product", methods=["POST"])
@auth_token
def add_product_api(email):
    return add_product_to_shop(email)
...
def get_products_by_tag_api(email, product_tag):
    ...
    return jsonify({"status": False, "message": "No products found", "data": None}), 404


@app.route("/comments/create", methods=["POST"])
@auth_token
def create_comment_api(email):
    return create_comment(email)


@app.route("/comments/<product>", methods=["GET"])
@auth_token
def get_comments_api(email, product):
    return get_comments(product)


@app.route("/comments", methods=["DELETE"])
@auth_token
def delete_comment_from_product_api(email):
    return delete_comment(email)


@app.route("/add-favorites", methods=["POST"])
@auth_token
def create_favorites_table_api(email):
    return add_favorites(email)


@app.route("/favorites", methods=["GET"])
@auth_token
def get_user_fav_products_api(email):
    return get_user_fav_products(email)


@app.route("/remove-favorites", methods=["DELETE"])
@auth_token
def delete_favorite_from_product_api(email):
    return delete_favorites(email)


@app.route("/recommend-crop", methods=["POST"])
@auth_token
def soil_classify_api(email):
    return recommend_crop()


@app.route("/chat", methods=["POST"])
@auth_token
def chatBot_api(email):
    return chatBot_controller(email)
