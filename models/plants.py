...
from helpers.upload_images import upload_images
...
def insert_plants_data_from_json(filename):
    ...
    for plant in plants_data:
        ...
        if not existing_plant:
            ...
            plant_image1 = upload_images(
                Path(f"./models/plants_data/plants_photos1/{plant_name}.png").resolve(),
                "plants_photos1",
            )
            plant_image2 = upload_images(
                Path(
                    f"./models/plants_data/plants_photos2/{plant_name}.jpeg"
                ).resolve(),
                "plants_photos2",
            )

            cursor.execute(
                "INSERT INTO plants (plantName, plantShortDescription, plantMediumDescription, plantLongDescription, minimumdegrees, stateofTemperature, numberOfWater, plantCareInstruct, plantImage1, plantImage2) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (
                    plant_name,
                    plant_short_description,
                    plant_medium_description,
                    plant_long_description,
                    minimum_degrees,
                    state_of_temperature,
                    number_of_water,
                    plant_care_instruct,
                    plant_image1,
                    plant_image2,
...
def get_plant_by_name(name):
    ...
    conn = create_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM plants WHERE plantName LIKE ?", ("%" + name + "%",))
    ...
    if data:
        ...
        conn.close()
        return jsonify(
            {"status": True, "message": "Plant fetched successfully", "data": plant}
        ), 200
    else:
        conn.close()
        return jsonify(
            {"status": False, "message": "Plant not found", "data": None}
        ), 404
...
def get_all_plants():
    ...
    if data:
        ...
        conn.close()
        return jsonify(
            {"status": True, "message": "Plants fetched successfully", "data": plants}
        ), 200
    else:
        conn.close()
        return jsonify(
            {"status": False, "message": "No plants found", "data": None}
        ), 404
