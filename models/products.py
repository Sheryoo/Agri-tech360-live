...
def add_product(name, price, description, seller, image, images, tags):
    ...
    images.append(image)
    cursor.execute(
        "INSERT INTO products (name, price, description, seller, image, images, tags) VALUES (?, ?, ?, ?, ?, ?, ?)",
...
def get_all_products():
    ...
    if products:
        ...
        cursor.close()
        return jsonify(
            {"status": True, "message": "Products fetched successfully", "data": data}
        )
    conn.close()
...
def get_products_by_tag(tag):
    ...
    conn = create_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM products WHERE tags LIKE ?", ("%" + tag + "%",))
    products = cursor.fetchall()
    data = []
    if products:
        for i in range(len(products)):
